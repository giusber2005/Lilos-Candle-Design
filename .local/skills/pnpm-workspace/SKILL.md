...
- Should be usually handled via `create-artifact` unless no artifact template satisfies the user's requirements.
...
If you are creating or updating an artifact, follow the `create-artifact` skill for the `presentArtifact` and `suggestDeploy()` lifecycle instead of redefining it here.
